...
//   利用页码获取文章数据
  router.post('/getArticle',function(req, res, next) {
      function getArticleData() {
        const sql =`select article.*,user.realName,user.avatar from user inner join article on user.id = article.user_id LIMIT ${(req.body.page-1)*10},${(req.body.page-1)*10+10}`;
